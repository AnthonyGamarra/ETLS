    ...
    query = f"""
            WITH ctsvi_filtrado AS (
                SELECT 
                    ...
                    SigVitRegSecNum,
                    ROW_NUMBER() OVER (
                        PARTITION BY SIGVITORICENASICOD, SIGVITCENASICOD, SIGVITACTMEDNUM
                        ORDER BY SigVitRegSecNum DESC
                    ) AS rn
                FROM SGSS.CTSVI10
                WHERE sigvittalla IS NOT NULL
                AND sigvitpeso IS NOT NULL
                AND SigVitPreArtSist IS NOT NULL
                AND SIGVITTEMP IS NOT NULL
                AND SigVithorfec >= TO_DATE('{start_mes.strftime('%d-%m-%Y')}', 'DD-MM-YYYY')
                AND SigVithorfec <  TO_DATE('{(end_mes + timedelta(days=1)).strftime('%d-%m-%Y')}', 'DD-MM-YYYY')
            )
            SELECT 
