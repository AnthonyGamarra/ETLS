...
table_name = "dwsge.dwe_consulta_externa_programacion_nm"
for mes in range(1,2):
    mes_str = f"{mes:02d}"
    try:
        cur_dst = conn_dst.cursor()
        partition_name = f"dwsge.dwe_consulta_externa_programacion_nm_{anio}_{mes_str}"
    ...
    try:
        query = f"""
                    ...
                    WHERE 
                        a.oricenasicod IN ('1','2','3','4','5','6','7')
                        AND a.properestregcod = '1'
                        AND a.servhoscod ='E21'
                        AND a.actcod ='B1'
                        AND a.actespcod ='005'
