...
# ==============================
# 6. Ciclo para extraer y copiar mes a mes (mes completo)
# ==============================
for start_mes, end_mes in month_range(start_date, end_date):
    ...
    query = f"""
                ...
                WHERE  x.resexafec        >= TO_DATE('{start_mes.strftime('%d-%m-%Y')}','DD-MM-YYYY')
                AND x.resexafec        <  TO_DATE('{end_mes.strftime('%d-%m-%Y')}','DD-MM-YYYY')
                and y.tipexacod = 1
                ORDER BY x.resexacenasicod
    """

    print(f"Ejecutando query para el mes {start_mes.strftime('%Y-%m')} en Oracle...")
    ...
    print(f"Cargando datos a PostgreSQL para el mes {start_mes.strftime('%Y-%m')}...")
    try:
        cursor_pg.copy_expert(
            sql=f"COPY {tabla_destino} ({', '.join(df.columns)}) FROM STDIN WITH CSV",
            file=csv_buffer
        )
        print("Mes cargado correctamente.")
    except Exception as e:
        print(f"Error al cargar el mes: {e}")
# ==============================
