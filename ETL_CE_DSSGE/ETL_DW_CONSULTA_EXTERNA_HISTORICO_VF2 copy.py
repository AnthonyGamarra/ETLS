...
# ==============================
# 5. Parámetros de fechas (rango semanal o cualquier rango)
# ==============================
...
for start_mes, end_mes in month_range(start_date, end_date):
    ...
    # ==============================
    # ELIMINAR SOLO LOS REGISTROS DEL RANGO DE FECHAS
    # ==============================
    fechas_rango = [start_mes + timedelta(days=i) for i in range((end_mes - start_mes).days + 1)]
    fechas_str   = [f.strftime('%d/%m/%Y') for f in fechas_rango]
    placeholders = ', '.join(['%s'] * len(fechas_str))
    print(f"Eliminando {len(fechas_str)} días del rango en {tabla_destino}...")
    try:
        cursor_pg.execute(f"DELETE FROM {tabla_destino} WHERE fecha_atencion IN ({placeholders})", fechas_str)
        print(f"Registros eliminados correctamente en {tabla_destino}.")
    except Exception as e:
        print(f"⚠️ Error al eliminar registros en {tabla_destino}: {e}")
